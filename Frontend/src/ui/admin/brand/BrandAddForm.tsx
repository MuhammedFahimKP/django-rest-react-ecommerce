...
const BrandAddFrom = ({ form, handleForm }: Props) => {
  ...
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      ...
      .max(20, "brand than 20 or 20  characters "),
  });
  ...
  function handleSubmit(value: IntialValues, actions: any) {
    ...
    apiClient
      .post<AdminCategory>(`admin/brand/`, value)
      .then((res) => {
        dataAddedContext?.mutate();
        res.status === 201 &&
          toast.custom((t) => (
            <SuccessAlert successText="Brand Added" toast={t} />
          )) &&
          actions.resetForm({
            values: {
              name: "",
  ...
  return (
    <Dialog placeholder={"name"} open={form} handler={handleForm} size="xs">
      <DialogHeader placeholder={"dailogou"}>Add Brand</DialogHeader>
