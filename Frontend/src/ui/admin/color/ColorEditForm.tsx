...
import { validColorName } from "../../../utils/color";
...
const ColorEditForm = ({ form, handleForm, name, id, onSuccess }: Props) => {
  const context = useContext(ToastContext);

  const validationSchema = Yup.object().shape({
    name: Yup.string()
      .required("please provide a name")
      .min(3, "color  must be 3 charaters ")
      .max(20, "color  20 or 20  charaters "),
  });
  ...
  function handleSubmit(value: IntialValues) {
    ...
    if (validColorName(formike.values.name) === false) {
      formike.setErrors({
        ...formike.errors,
        name: "please provide valid color",
      });
    ...
    apiClient
      .patch<AdminBrand>(`admin/color/${id}/`, value)
      .then((res) => {
        res.status === 200 &&
          toast.custom((t) => (
            <SuccessAlert successText="Color Upddated" toast={t} />
          )) &&
  ...
  useEffect(() => {
    return () => {
      context?.anotherToast && context?.removeAnotherToast();
    };
  }, []);

  return (
    <Dialog placeholder={"name"} open={form} handler={handleForm} size="xs">
      <DialogHeader placeholder={"dailogou"}>Edit Color</DialogHeader>
