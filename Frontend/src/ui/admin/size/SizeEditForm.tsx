...
const SizeEditForm = ({ form, handleForm, name, id, onSuccess }: Props) => {
  ...
  function handleSubmit(value: IntialValues) {
    ...
    apiClient
      .patch<AdminSize>(`admin/size/${id}/`, value)
      .then((res) => {
        res.status === 200 &&
          toast.custom((t) => (
            <SuccessAlert successText="Size Updated" toast={t} />
          )) &&
  ...
  return (
    <Dialog placeholder={"name"} open={form} handler={handleForm} size="xs">
      <DialogHeader placeholder={"dailogou"}>Edit Size</DialogHeader>
