...
import BrandRow from "../../ui/admin/brand/BrandRow";
import { usePaginatedAdminBrand } from "../../hooks";
...
const BrandList = ({ filteringParams }: Props) => {
  ...
  const handleDelete = (id: string) => {
    setData((prevData: AdminBrand[]) => {
      const newData = prevData.filter((item: AdminBrand) => item.id !== id);
      return newData;
    });
  };
        ...
        <CardBody placeholder={undefined} className="overflow-scroll px-0 ">
          <table className="mt-4 w-full min-w-max table-auto text-left">
            ...
            <tbody>
              {data.map(
                (
                  { id, created, updated, name, is_active }: AdminBrand,
                  index: number
                ) => {
